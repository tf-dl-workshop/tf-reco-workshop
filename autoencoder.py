...
train_arr, test_arr = dataprep.get_train_test()

train_rating = {'ratings': train_arr.astype(np.float32)}
test_eval_rating = {'ratings': train_arr.astype(np.float32), 'targets': test_arr.astype(np.float32)}
train_eval_rating = {'ratings': train_arr.astype(np.float32), 'targets': train_arr.astype(np.float32)}
...
def auto_encoder(features, targets, mode, params):
    ...
    with tf.name_scope("encoder"):
        ...
        encoding_tensor = tf.sigmoid(tf.sparse_tensor_dense_matmul(sparse_ratings, encoder_w)) + encoder_b
    ...
    with tf.name_scope("decoder"):
        ...
        decoding_tensor = tf.identity(tf.matmul(encoding_tensor, decoder_w)) + decoder_b

    loss = None
    if mode == model_fn_lib.ModeKeys.TRAIN:
        with tf.name_scope("loss"):
            # backpropagate only partial observed ratings
            neg_sparse_decoding = tf.SparseTensor(idx, tf.negative(tf.gather_nd(decoding_tensor, idx)),
                                                  decoding_tensor.get_shape())
    ...
    if mode == model_fn_lib.ModeKeys.EVAL:
        with tf.name_scope("eval_loss"):
            ...
            neg_sparse_decoding = tf.SparseTensor(target_idx, tf.negative(tf.gather_nd(decoding_tensor, target_idx)),
                                                  decoding_tensor.get_shape())
...
with tf.Session() as sess:
    ...
    validation_monitor = monitors.ValidationMonitor(input_fn=test_eval_input_fn, eval_steps=1, every_n_steps=500,
                                                    name='test')
    train_monitor = monitors.ValidationMonitor(input_fn=train_eval_input_fn, eval_steps=1, every_n_steps=500,
                                               name='train')

    autoencoder_cf = Estimator(
        model_fn=auto_encoder,
        params=model_params,
        model_dir='model/_summary/auto_rec',
